...
ls=[0.8,0.7]';
theta0=[3*pi/4, pi/2, pi/2]; %Choose some random starting point.
n=100;
mode = 1;
%Start position
desired=[-0.2, -0.2, 0.5];
...
while obj_picked <= n_cans
    clf;
    hold on;
  
    for current_segment=1:steps
        clf;
        hold on;
        desired_pos = bezier(start_can_pos(obj_picked,:) , (end_can_pos(obj_placed+1 , :)),steps,current_segment);
     
        t = invKin3D(ls, t', desired_pos, n, mode);
        [pos_hand, J] = evalRobot3D(ls, t');
        plotCylinderWithCaps(0.1, pos_hand, 0.2, 12, [1 0 0], 'z');
        plot_scene(obj_picked, obj_placed, start_can_pos, end_can_pos, gca, ls, t);
        drawnow()
    ...
    if obj_picked < n_cans

        for current_segment=1:steps
            clf;
            hold on;
            desired_pos = bezier((end_can_pos(obj_placed + 1 , :)),start_can_pos(obj_picked + 1,:) ,steps,current_segment);
            t = invKin3D(ls, t', desired_pos, n, mode);
            plotCylinderWithCaps(0.1, pos_hand, 0.2, 12, [1 0 0], 'z');
            plot_scene(obj_picked, obj_placed, start_can_pos, end_can_pos, gca, ls, t');
            drawnow();
